package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_367 >= var_280.length)
         {
            return "";
         }
         var petColor:IPetColor = var_280[var_367] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_509[var_1342] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_367 = param1.index;
         if(var_367 < 0 || var_367 > var_280.length)
         {
            var_367 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_902.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_205,onTextWindowEvent);
         var_902 = new Map();
         ...
         var_735 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_280 = [];
         var_509 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_735);
               if(_loc11_)
               {
                  var_509 = _loc11_.breeds;
                  var_280 = _loc11_.colors;
                  if(var_280 == null)
                  {
                     var_280 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_509.length)
         {
            _loc12_ = var_509[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_735,_loc12_.id),getRaceLocalizationKey(var_735,_loc12_.id)));
            _loc6_++;
         }
         var_902.add(var_735,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_936,onApproveNameResult);
         _loc5_ = var_902.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_280)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_902.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_367 >= 0 && var_367 < var_280.length)
         {
            _loc7_ = var_280[var_367] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_509[var_1342] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_735,_loc3_.id,_loc2_,AvatarScaleType.const_50);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_318,AvatarAction.const_589);
